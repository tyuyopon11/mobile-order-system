...
export default async function MemberDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div>
    ...
    {isShopManager ? <ShopManagerControls memberId={member.id} currentShopId={member.shop_id} approvalStatus={member.approval_status} isActive={member.is_active} shops={shops ?? []} /> : <section className="mt-6 rounded-2xl border border-stone-200 bg-white p-6 text-sm text-stone-500">ショップ管理者向けの管理操作は、会員種別がshopの会員にのみ表示されます。</section>}
  </div>;
}
