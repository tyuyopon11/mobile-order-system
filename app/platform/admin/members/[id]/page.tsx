...
export default async function MemberDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div>
    <section className="rounded-[28px] border border-stone-200 bg-white px-6 py-8 shadow-sm sm:px-9 sm:py-10">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3"><h1 className="text-3xl font-semibold text-stone-900">{member.name}</h1><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-blue-700">{member.role}</span><span className={`rounded-full px-3 py-1 text-xs font-medium ${member.is_active ? "bg-green-100 text-green-800" : "bg-stone-100 text-stone-600"}`}>{member.is_active ? "利用中" : "利用停止"}</span></div>
    </section>
    ...
    {isShopManager ? <ShopManagerControls memberId={member.id} currentShopId={member.shop_id} isActive={member.is_active} shops={shops ?? []} /> : <section className="mt-6 rounded-2xl border border-stone-200 bg-white p-6 text-sm text-stone-500">ショップ管理者向けの管理操作は、会員種別がshopの会員にのみ表示されます。</section>}
  </div>;
}
